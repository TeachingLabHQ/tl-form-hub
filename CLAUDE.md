...
The coach log's questions and show/hide rules are specified in a Google Sheet, exported to `app/components/coach-log/spec/coach-log.spec.json` by `Rscript scripts/spec/export_spec.R`. Tests fail when the code drifts from it, so a question/option/rule/column change means updating the sheet and re-exporting too. See the README section "Coach Log Question Spec".
